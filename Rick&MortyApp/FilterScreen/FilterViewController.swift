...
class FilterViewController: UIViewController {
    ...
    private func setUI() {
        ...
        setSelectedImage(Filter: viewModel.characterFilter.value!)
    }
...
//            firstFilterImageView.image = UIImage(named: "filterUnselectedIcon")
    ...
    private func setSelectedCharacter(Filter: Filters) {
        ...
        setSelectedImage(Filter: Filter)
        viewModel.characterFilter.value = Filter
    }

    @objc private func filterButtonPressed(sender: UIButton) {

        let currentFilter = viewModel.characterFilter.value!
        guard let selectedFilter = TagToFilter[sender.tag] else { return }
        if currentFilter == selectedFilter {
            setSelectedCharacter(Filter: .allCharacters)
        } else {
            setSelectedCharacter(Filter: selectedFilter)
        }
...
//                setSelectedCharacter(Filter: .mortys)
//            }
//        case .mortys:
//            if sender.tag == 1 {
//                setSelectedCharacter(Filter: .ricks)
//            }else if sender.tag == 2 {
//                setSelectedCharacter(Filter: .allCharacters)
//            }
//        case .allCharacters:
